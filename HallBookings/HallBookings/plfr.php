...
$rslt= mysql_fetch_array(mysql_query("select username,passcode from plc where username ='$user' and passcode='$pcde'"));

if($user==$rslt['username'] and $pcde==$rslt['passcode']){ //checking the user entered username with the database

$nrslt= mysql_query("update plc set password='$npass' where username='$user' and passcode='$pcde'"); // update password
...
echo "<script> alert('Oops Try Again!!!Passcode/Username Missmatched')</script>";
     ...
     $relt=  mysql_fetch_array(mysql_query("select username,password from plc where  username='$usr'and password='$ps'"));
     if($usr==$relt['username']and $ps==$relt['password']){
         $nrelt=mysql_query("update plc set passcode='$npco' where username='$usr'and password='$ps'");
