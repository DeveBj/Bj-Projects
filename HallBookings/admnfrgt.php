...
$rslt= mysql_fetch_array(mysql_query("select username,passcode from admin where username ='$user' and passcode='$pcde'"));

if($user==$rslt['username'] and $pcde==$rslt['passcode']){ //checking the user entered username with the database

$nrslt= mysql_query("update admin set password='$npass' where username='$user' and passcode='$pcde'"); // update password
       ...
       function func_fpcode(){
           ...
           if(!$found1){
            ...
            if(isset($_POST['cpass'])){
                ...
                elseif(!$npco){
                    echo  "<script> alert('Oops Try Again!!!Passcode Missmatched')</script>";
                }
 else {
     $relt=  mysql_fetch_array(mysql_query("select username,password from admin where  username='$usr'and password='$ps'"));
     if($usr==$relt['username']and $ps==$relt['password']){
         $nrelt=mysql_query("update admin set passcode='$npco' where username='$usr'and password='$ps'");
